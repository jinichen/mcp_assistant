...
class NVIDIAModel(LLMBase):
    ...
    def __init__(self, model_name: str = "mixtral-8x7b-instruct"):
        self.model_name = model_name
        self.api_key = os.getenv("NVIDIA_API_KEY")
        self.nvidia_api_url = os.getenv("NVIDIA_API_URL", "https://api.nvidia.com/v1/services")
        if not self.api_key:
    ...
    def _create_llm(self) -> ChatNVIDIA:
        """Create LangChain NVIDIA AI model"""
        if not self.api_key:
            raise ValueError("NVIDIA API key not found. Please set NVIDIA_API_KEY environment variable.")
    ...
    def available_models(self) -> List[str]:
        ...
        env_models = get_models_from_env("nvidia")
        if env_models:
            return env_models
    ...
    def get_available_models() -> List[str]:
        """Return list of available models (static method)"""
        # Try to get models from environment variable first
        env_models = get_models_from_env("nvidia")
        if env_models:
            return env_models
